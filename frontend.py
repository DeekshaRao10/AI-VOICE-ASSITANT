...
def fetch_doctors(dept=None):
    try:
        params = {"department": dept, "show_list": True} if dept and dept != "Any" else {"show_list": True}
        res = requests.get(f"{BASE_URL}/doctors", params=params)
        if res.status_code == 200:
            data = res.json()
            if "doctors" in data:
                return [d["name"] for d in data["doctors"]]
    except:
        pass
    return []
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    if "available_doctors" in data:
                        names = [d["name"] if isinstance(d, dict) else d for d in data["available_doctors"]]
                        st.info(f"👨‍⚕️ **Available Doctors:** {', '.join(names)}")
